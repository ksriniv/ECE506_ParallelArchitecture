...
Cache* cache_factory::create_cache(int protocol_no, int size, int assoc,
		int block_size, int num_processors, ulong directory_size,
		int processor_num) {
	switch ((protocol_name) protocol_no) {
	case MESI_dsm:
		return new MESI::MESI(size, assoc, block_size, num_processors,
				processor_num);
	default:
		return NULL;
	}
...
void Cache::print_stats(int processor_number, int protocol, int dir_type) {
	...
	cout << (int) get_E2M();
	cout << endl;
	cout << setw(2) << count++ << setw(40) << ". number of S2EM in directory: ";
	cout << (int) get_dir_S2EM();
	cout << endl;
	cout << setw(2) << count++ << setw(40) << ". number of EM2S in directory: ";
	cout << (int) get_dir_EM2S();
	cout << endl;
	/*
